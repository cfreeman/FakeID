...
navigator.mediaDevices.getUserMedia(constraints).then(function (stream) {
  ...
  video.onloadedmetadata = function (e) {
    eBackCanvas.width = iVideoWidth;
    eBackCanvas.height = iVideoHeight;
    video.play();
    render(true);
  };
}).catch(function (err) {
...
function render(firstPass) {
  console.log("render");
  ctx.clearRect(20, 50, iPhotoWidth, iPhotoHeight);
  ...
  if (firstPass) {
    ...
    sig_image.onload = function () {
      ctx.drawImage(sig_image, 240, 165);
    };
    ...
    bul_image.onload = function () {
      ctx.drawImage(bul_image, 19, 327);
    };
    ...
    lem_image.onload = function () {
      ctx.drawImage(lem_image, 245, 240);
    };
    ...
    nasa_image.onload = function () {
      ctx.drawImage(nasa_image, 235, 45);
    };
  }
  ...
  renderDetails(fName, lName, 372);
}

function renderDetails(sFirstName, sLastName, iYpos) {
  var sProperty = "PROPERTY OF THE TOM SACHS STUDIO";
  var rules = ["IF A GUEST IN YOUR LAIR ANNOYS YOU, TREAT HIM CRUELLY AND WITHOUT MERCY", "CHANGE WHAT YOU CAN", "DON'T BOTHER WHAT YOU CAN'T", "MENDING IS BETTER THAN ENDING, PRACTICE BRICOLAGE", "DO NOT KILL UNLESS YOU ARE ATTACKED OR FOR YOUR FOOD", "DO NOT HARM LITTLE CHILDREN", "AN ARTIST'S BEST WORK LIES JUST BEYOND HIS ABILITY TO UNDERSTAND IT", "WHEN IN ANOTHER'S LAIR, SHOW HIM RESPECT OR ELSE DO NOT GO THERE", "BE AFRAID AND DO IT ANYWAY", "ANYTHING BUT THE GIRL"];
